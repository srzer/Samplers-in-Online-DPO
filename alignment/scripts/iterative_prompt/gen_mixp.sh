...
if [ "$num_iter" -gt 2 ]; then
  infer_model="models/rlhflow_iter$((num_iter-1))_hybrid"
else
...
CUDA_VISIBLE_DEVICES=5 python ./generation/iterative_prompt/get_hf2.py --model_name_or_path ${infer_model} --ref_model_name_or_path ${ref_model} --dataset_name_or_path ${prompt_dir} --output_dir ${output_dir} --K ${K} --temperature ${temperature} --local_index 3 --my_world_size ${my_world_size} --eos_ids 2 --max_input_length ${max_input_length} --max_new_tokens ${max_new_tokens} --num_iter ${num_iter} --seed ${seed} 

wait
python ./generation/merge_data.py --base_path ${output_dir} --output_dir ./data/gen_data_iter${num_iter}_hybrid_temp2.json --num_datasets ${my_world_size}